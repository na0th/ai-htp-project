...
/*집 분류 모델*/
insert into house_type(id, result)
values(0, "일반 집:- 가장 전형적인 그림으로 집 유형에 대한 해석보다 지붕, 창문 등의 각 요소에 대한 해석이 중요하다.");

insert into house_type(id, result)
values(1, "현대적인 집 (아파트):- 이러한 그림은 초등학생이 많이 그리는 그림입니다. 만일 당신이 성인이라면, 현실적이고 아이큐가 높을 수 있습니다.");

insert into house_type(id, result)
values(2, "나무 위의 집:- 나무 위에 집을 그린 당신은, 자기를 보호하고자 하는 욕구가 강한 편입니다. 당신은 열악한 환경에서 안전한 대피소를 찾고 있는 상태입니다.");

insert into house_type(id, result)
values(3, "특수한 집 (교회, 절, 학교 등):- 특수한 집 형태를 그린 피검자는 보통 자신의 가정에 대해 불만이나 갈등을 가지고 있는 경우가 많은 것으로 해석됩니다. 또한, 특수한 집이 피검자의 자화상을 나타내거나 특별한 관심을 나타내기도 합니다. 
